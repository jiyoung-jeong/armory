...
class SyncBroker(ActionChunkBroker):
    ...
    def __init__(
        ...
        resolved_execution_horizon = (
            server_action_horizon if execution_horizon <= 0 else int(execution_horizon)
        )
    ...
    def _infer(self, obs: Observation) -> None:
        ...
        self._ws_client.send(
            obs, self.deadline, self._next_action_step, execution_horizon=self.execution_horizon
        )
