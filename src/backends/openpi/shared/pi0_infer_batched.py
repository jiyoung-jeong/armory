...
def scaled_matmul_small_bias_res(
    ...
    for p in range(pid, grid_i * grid_j, psize):
        ...
        norm_factor = tl.load(
            inp_norm_factor_ptr + i + tl.arange(0, BLOCK_SIZE_N), mask=i + tl.arange(0, BLOCK_SIZE_N) < seq_len, other=0
        )
        acc = tl.load(
            res_ptr + (i + tl.arange(0, BLOCK_SIZE_N))[:, None] * hidden + (j + tl.arange(0, BLOCK_SIZE_M))[None, :],
            mask=((i + tl.arange(0, BLOCK_SIZE_N))[:, None] < seq_len)
...
def merge_split_k_bias_res(
    ...
    for i in range(pid * BLOCK, seq_len * hidden, psize * BLOCK):
        acc = tl.load(
            res_ptr + i + tl.arange(0, BLOCK), mask=(i + tl.arange(0, BLOCK)) < (seq_len * hidden), other=0.0
        ).to(tl.float32)
        acc += tl.load(
...
def layer_norm_small_kernel(
    x_ptr, out_ptr, norm_w_ptr, norm_b_ptr, seq_len: tl.constexpr, features: tl.constexpr, eps: tl.constexpr = 1e-5
):
    pid = tl.program_id(0)
    psize = tl.num_programs(0)
    ...
    for i in range(pid, seq_len, psize):
        ...
        tl.store(
            out_ptr + i * features + tl.arange(0, MAX_LEN), x.to(tl.bfloat16), mask=tl.arange(0, MAX_LEN) < features
        )
...
def layer_norm_n256_1152(x, norm_w, norm_b, out):
    ...
    layer_norm_small_kernel[seq_len,](x, out, norm_w, norm_b, seq_len=seq_len, features=1152)


def layer_norm_QKV_matmul_n256_1152_3456_bias(x, norm_w, norm_b, qkv_w, qkv_b, out, x_norm):
    num_views = x.shape[0]
    seq_len = 256 * num_views
    layer_norm_small_kernel[seq_len,](x, x_norm, norm_w, norm_b, seq_len=seq_len, features=1152)
...
def matmul_512x1152x1152_twopart_bias_res(
    ...
    for p in range(pid, grid_i * grid_j, psize):
        ...
        acc = tl.load(
            old_ptr + (i + tl.arange(0, BLOCK_SIZE_N1))[:, None] * hidden + (j + tl.arange(0, BLOCK_SIZE_M1))[None, :]
        ).to(tl.float32)
    ...
    for p in range(pid, grid_i * grid_j * grid_k, psize):
        ...
        tl.store(
            out2_ptr
            + (i + tl.arange(0, BLOCK_SIZE_N2))[:, None] * ((hidden - hidden1) * grid_k)
...
def combine_1536_1152_twopart(out_ptr, inp_ptr, seq_len: tl.constexpr, hidden: tl.constexpr):
    ...
    for i in range(pid * 2, seq_len, psize * 2):
        ...
        for j in range(1, 4):
            inp += tl.load(inp_ptr + (i + tl.arange(0, 2)[:, None]) * 128 * 4 + tl.arange(0, 128) + 128 * j).to(
                tl.float32
            )
...
def layer_norm_matmul_n256_1152_4304_bias_gelu(x, norm_w, norm_b, weight, bias, out, x_norm):
    ...
    matmul_small_bias_gelu[
        ((seq_len + BLOCK_SIZE_N - 1) // BLOCK_SIZE_N) * ((4304 + (BLOCK_SIZE_M - 1)) // BLOCK_SIZE_M),
    ](
        x_norm,
        weight,
        out,
...
def rms_matmul_n_2048_16384_gate(x, weight1, weight2, out, x_norm):
    ...
    matmul_small_gate[((seq_len + 127) // 128, (16384 + 63) // 64)](x_norm, weight1, weight2, out, seq_len, 2048, 16384)
...
def layer_norm_matmul_n256_1152_2048_bias(x, norm_w, norm_b, proj_w, proj_b, out, x_norm):
    ...
    layer_norm_small_kernel[seq_len,](x, x_norm, norm_w, norm_b, seq_len=seq_len, features=1152, eps=1e-5)
    ...
    for i in range(pid1 * BLOCK_SIZE_N, seq_len, psize1 * BLOCK_SIZE_N):
        for j in range(pid2 * BLOCK_SIZE_M, (num_head + 2) * head_dim, psize2 * BLOCK_SIZE_M):
            ...
            for k in range(0, features, BLOCK_SIZE_K):
                ...
                w = tl.load(
                    weight_QKV_ptr
                    + (k + tl.arange(0, BLOCK_SIZE_K)[:, None]) * ((num_head + 2) * head_dim)
                    + j
...
def transformer_encoder(weights, buffers, encoder_seq_len):
    ...
    for i in range(18):
        ...
        if i != 17:
            ...
            matmul_n_2048_2048_res(attn, weights["encoder_attn_o_w"][i], buffers["encoder_x"])
            ...
            matmul_n_16384_2048_res(buffers["encoder_hidden"], weights["encoder_ffn_down_w"][i], buffers["encoder_x"])
...
def matvec_bias_kernel(x_ptr, weight_ptr, bias_ptr, out_ptr, features: tl.constexpr, hidden: tl.constexpr):
    ...
    for j in range(pid * BLOCK_SIZE_M, hidden, psize * BLOCK_SIZE_M):
        ...
        for k in range(0, features, BLOCK_SIZE_N):
            x = tl.load(
                x_ptr + k + tl.arange(0, BLOCK_SIZE_N), mask=(k + tl.arange(0, BLOCK_SIZE_N) < features), other=0.0
            )
            w = tl.load(
                weight_ptr
        ...
        tl.store(
            out_ptr + j + tl.arange(0, BLOCK_SIZE_M), acc.to(tl.bfloat16), mask=j + tl.arange(0, BLOCK_SIZE_M) < hidden
        )


def matmul_1_32_1024_bias(x, weight, bias, out):
    matvec_bias_kernel[((1024 + 7) // 8,)](x, weight, bias, out, features=32, hidden=1024)
...
def scaled_matmul_rope_qkv(
    ...
    while pid < grid_m * grid_n:
        ...
        for k in range(0, features, BLOCK_SIZE_K):
            ...
            x = tl.load(inp_ptr + offs_i * features + offs_k[None, :], mask=offs_k[None, :] < features, other=0)
            # x = x * norm_factor[:, None]
            w = tl.load(
...
def softmax_kernel_mask0(
    ...
    for i in range(pid * BLOCK_SIZE_M, queries, psize * BLOCK_SIZE_M):
        ...
        vals = tl.load(inp_ptr + offs_i * keys + offs_j, mask=attn_mask, other=-float("inf"))
        ...
        tl.store(out_ptr + offs_i * keys + offs_j, vals, mask=(offs_i < queries) & (offs_j < keys))
...
def matmul_k8_256_n_softmax_mask0(Q, K, out, encoder_seq_len):
    ...
    softmax_kernel_mask0[((total_queries + 3) // 4,)](
        out, total_queries, total_keys, 8, encoder_seq_len, out, BLOCK_SIZE_M=4, BLOCK_SIZE=1024
    )
    ...
    for step in range(10):
        ...
        for i in range(18):
            ...
            matmul_k8_256_n_softmax_mask0(
                buffers["decoder_q_buf"], buffers["encoder_K"][i], buffers["decoder_attn_buf"], encoder_seq_len
            )
            matmul_k8_n_256(buffers["decoder_attn_buf"], buffers["encoder_V"][i], buffers["decoder_q_buf"])

            matmul_k_2048_1024_res(
                buffers["decoder_q_buf"].view(-1, 2048), weights["decoder_attn_o_w"][i], buffers["decoder_x"]
            )
            ...
            matmul_k_4096_1024_res(buffers["decoder_hidden"], weights["decoder_ffn_down_w"][i], buffers["decoder_x"])
    ...
    def __init__(self, checkpoint, num_views, chunk_size):
        ...
        self.buffers = {
            ...
            "decoder_attn_buf": torch.empty(
                (decoder_seq_len * 8, encoder_seq_len + decoder_seq_len), dtype=torch.bfloat16, device="cuda"
            ),
