...
Run:
    uv run python scripts/visualization/generate_latex_tables.py \\
        <summary-dir>
"""
...
def _cell_single(
    row: pd.Series, cols, scenario: str, sched: str, metric: str, decimals: int, scale: float
...
def _cell_pair(
    ...
    text = f"{_fmt_num(a, decimals, scale)} / {_fmt_num(b, decimals, scale)}"
...
def _build_table(
    ...
    for scenario in SCENARIO_ORDER:
        if scenario not in SCENARIO_DISPLAY:
            continue
        schedulers = _present_schedulers(df, cols, scenario, require_metrics_per_scenario[scenario])
        if schedulers:
    ...
    for nr, row in df.iterrows():
        ...
        for _scenario, scheds in scenarios_used:
            group_results = [spec.cell_fn(row, cols, _scenario, sched) for sched in scheds]
    ...
    for scenario, scheds in scen_scheds:
        ...
        for nr in df.index:
            if num_robots_filter is not None and int(nr) not in num_robots_filter:
                continue
            row = df.loc[nr]
            if any(_get_raw(row, cols, scenario, sch, "thr_total") is not None for sch in scheds):
                nrs.append(int(nr))
        if not nrs:
            continue
        ...
        for i, nr in enumerate(nrs):
            row = df.loc[nr]
            if i == 0:
                scen_cell = f"\\SetCell[r={n_rows}]{{}} {scen_label}" if n_rows > 1 else scen_label
            else:
                scen_cell = ""
...
def main() -> None:
    ...
    thr_unit_suffix = (
        " ($\\times 100$)"
        if abs(thr_scale - 100.0) < 1e-9
        else ("" if abs(thr_scale - 1.0) < 1e-9 else f" ($\\times {thr_scale:g}$)")
    )
        ...
        return [
            (
                TableSpec(
                    name="avg_starvation",
                    caption=(
                        "Average starvation as the number of robots increases"
